...
static class Demo
{
    public static bool VerifySetup(string rootPath, out string? token)
    {
        ...
        if (!FileExists(Path.Combine(rootPath, "database", "database.sqlproj"), out var error))
        {
            ShowError($"{error} (SQL Database Project for schema deployment)");
            return false;
        }

        var configJs = Path.Combine(rootPath, "web-app", "config.js");
        ...
        var dabConfig = Path.Combine(rootPath, "data-api", "dab-config.json");
        ...
        {
            ShowError($"{error} (run azure-infra/entra-setup.ps1 to configure)");
    ...
    private static bool HasPrerequisites(out string? error)
    {
        ...
        var msg =
            ...
            "with a [white]client secret[/] for the OBO token exchange\n" +
            "before the app can start. The setup script automates this:\n\n" +
            "  [white]az login[/]\n" +
            "  [white]./azure-infra/entra-setup.ps1[/]\n\n" +
            (hasMissing
    ...
    {
        var scriptPath = Path.Combine(rootPath, "azure-infra", "entra-setup.ps1");
        AnsiConsole.Markup("  [yellow]Running:[/] ./azure-infra/entra-setup.ps1\n\n");

        var success = TryRun("pwsh", $"-NoProfile -ExecutionPolicy Bypass -File \"{scriptPath}\"", rootPath);
        error = success ? null : "entra-setup.ps1 failed. Check the output above.";
        return success;
    }

    private static string? ReadToken(string rootPath)
    {
        var azureEnvFile = Path.Combine(rootPath, ".azure-env");
        if (!File.Exists(azureEnvFile)) return null;

        foreach (var line in File.ReadAllLines(azureEnvFile))
        {
