...
// ──────────────────────────────────────
// DAB Container App (OBO — user-delegated auth to SQL)
// ──────────────────────────────────────

// SQL Auth connection string — used by SQL Commander (no SAMI/OBO)
...
// Bare connection string for DAB with OBO — MUST NOT include Authentication= keyword.
// Microsoft.Data.SqlClient throws if AccessToken is set AND Authentication= is present.
// DAB 2.0 uses Azure.Identity (MSI) for health checks and injects the OBO token per-request.
...
resource dabApp 'Microsoft.App/containerApps@2024-03-01' = {
  ...
  properties: {
    ...
    configuration: {
      ...
      secrets: [
        { name: 'db-conn', value: sqlBareConnString }
