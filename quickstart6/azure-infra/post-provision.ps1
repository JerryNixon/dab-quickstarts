...
$oboClientSecret   = $env:AZURE_OBO_CLIENT_SECRET
...
# ── 4. Grant DAB managed identity access to database (fallback for non-OBO requests) ──
...
# ── 5. Grant signed-in user database access (OBO will connect as this user) ──

Write-Host "Granting signed-in user database access for OBO..." -ForegroundColor Yellow
$grantUserSql = @"
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = '$($currentUser.upn)')
BEGIN
    CREATE USER [$($currentUser.upn)] FROM EXTERNAL PROVIDER;
END;
ALTER ROLE db_datareader ADD MEMBER [$($currentUser.upn)];
ALTER ROLE db_datawriter ADD MEMBER [$($currentUser.upn)];
"@
Invoke-Sqlcmd -ServerInstance $sqlServerFqdn -Database $sqlDb -AccessToken $accessToken -Query $grantUserSql
Write-Host "User $($currentUser.upn) granted read/write" -ForegroundColor Green

# ── 6. Update dab-config.json with real auth values, CORS, and enable OBO ──

Write-Host "Updating DAB config with EntraId auth, CORS, and OBO..." -ForegroundColor Yellow
...
Pop-Location
Write-Host "DAB config updated (OBO enabled)" -ForegroundColor Green

# ── 7. Build and push DAB image to ACR ──
...
# ── 8. Update DAB container app with custom image + OBO env vars ──

Write-Host "Updating DAB container app with OBO configuration..." -ForegroundColor Yellow

# Add OBO client secret as a container app secret
az containerapp secret set `
    --name $dabAppName `
    --resource-group $resourceGroup `
    --secrets "obo-secret=$oboClientSecret" | Out-Null
...
# ── 9. Add Azure redirect URI to app registration ──
...
# ── 10. Generate config.js and deploy web files ──
...
# ── 11. Update web container app with custom image ──
...
# ── 13. Update MCP Inspector container app ──
...
# ── 14. Update local config.js for dev ──
...
# ── 15. Upsert repo-root .github/mcp.json entry for quickstart6 ──

$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "../..")).Path
$mcpDir = Join-Path $repoRoot ".github"
$mcpFile = Join-Path $mcpDir "mcp.json"
$mcpServerName = "azure-sql-mcp-qs6"
...
Write-Host ""
Write-Host "OBO is ENABLED — SQL Server sees the actual user identity." -ForegroundColor Green
Write-Host "Test with: GET $apiUrlAzure/api/WhoAmI (with bearer token)" -ForegroundColor Yellow
